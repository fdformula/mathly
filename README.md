# MathLua is a Tiny, Portable, Free but Powerful MATLAB and More

The heart of MathLua is Mathly, a [Lua](https://www.lua.org) module that turns Lua into a tiny, portable, free but powerful MATLAB and more. It provides a group of commonly
...
MathLua uses Plotly JavaScript graphing tools (see https://plotly.com/javascript/) to plot graphs. Therefore, graphs and animations are
...
## Which version of Lua is supported? Is there any IDE for MathLua?
...
Two text editors, [CudaText](https://github.com/fdformula/mathly/tree/main/IDE%20-%20CudaText) and [Lite XL](https://github.com/fdformula/mathly/tree/main/IDE%20-%20Lite%20XL), are each a very good "IDE" for MathLua. You may download the editors there with Lua and Mathly included and integrated.
...
`..` (or `horzcat`), `all`, `any`, `apply`, `cc`, `clc`, `clear`, `copy`, `corr`, `cross`, `demathlua` (or `demathly`), `det`, `diag`, `diff` (or `diff1`), `diff2`, `disp`, `display`, `dot`,
...
`inv`, `isempty`, `isinteger`, `iseven`, `isodd`, `ismatrix`, `ismember`, `isvector`, `lagrangepoly`, `length`, `linsolve`, `linspace`, `lu`, `map`, `map1`, `match`, `mathlua` (`mlua`, or `mathly`), `max`, `mean`,
