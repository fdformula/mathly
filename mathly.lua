...
HOME PAGE
	...
	https://mathlua.github.io, https://github.com/fdformula/mathly
...
-- eq, le, ge, lt, gt: behave like ==, <=, >=, <, > in MATLAB for matrics
-- NO meaningful ==, <=, >=, <, > on tables in Lua b/c results are "always converted to a boolean" (see: manual.html#2.4)
local function _eq_(x, y)
	local v
	if math.type(x) == 'integer' and math.type(y) == 'integer' then
		v = x == y
	else
		v = math.abs(x - y) < 100*eps
	end
...
function rand(r, c) if r == nil then return math.random() else return _create_table(r, c, 'random') end end
...
function randi(imax, m, n)
	...
	assert(isinteger(imin) and isinteger(imax), 'randi(imax, m, n), randi({imin, imax}, m, n): imin and imax must be integer.')
...
local function _write_manipulate_html(fname, cs, rs, xr, yr, tr, title, xexpr, yexpr, jxexpr, jyexpr, enhancements, jscode, opts)
	...
	local s = [[<!DOCTYPE html><!-- generated by MathLua: https://mathlua.github.io -->
<html>
<head>
<meta charset="utf-8">
...
-- rows have to be the same, e.g.: #A == #B
function horzcat(...)
	local args = {}
	for _, v in pairs{...} do
		args[#args + 1] = v
...
-- columns have to be the same; e.g.: #A[1] == #B[1]
function vertcat(...)
	local args = {}
	for _, v in pairs{...} do
		args[#args + 1] = v
...
mathly_meta.__add = function(A, B)
	return mathly.add_sub_shared(A, B, '+')
end

mathly_meta.__sub = function(A, B)
	return mathly.add_sub_shared(A, B, '-')
end
...
mathly_meta.__mul = function(A, B)
	return mathly.mul(A, B)
end

-- scalar multiplication
function mathly.mulnum(A, num)
	local t = {}
	for i = 1,#A do
		if type(A[1]) == 'table' then
			t[i] = {}
...
function mathly.pow(A, n)
	...
	elseif #A == 1 then -- row vector, element wise
		for i = 1, #A[1] do t[i] = A[1][i] ^ n end
		t = {t}
	elseif #A[1] == 1 then -- column vector
		for i = 1, #A do t[i] = { A[i][1] ^ n } end
	else
		assert(#A == #A[1], "A ^ n: A must be a square matrix.")
		if n == 0 then return setmetatable(eye(#A), mathly_meta) end
		t = copy(A)
		for i = 2, n	do t = mathly.mul(t, A) end
	end
	return _set_matrix_meta(t)
end
...
mathly_meta.__pow = function(A, opt)
	if opt == 'T' then
		...
		return setmetatable(mathly.pow(A, opt), mathly_meta)
	end
end
